...
def parse_vdp(vid, url):
    ...
    # Meta vehicle-catalog drivetrain enum: 4X2, 4X4, AWD, FWD, RWD (leave
    # blank when unknown — an empty optional field is always safe).
    drive_raw = label_value(text, "Drive Train").upper()
    drivetrain = ""
    if "AWD" in drive_raw:
        drivetrain = "AWD"
    elif "FWD" in drive_raw:
        drivetrain = "FWD"
    elif "RWD" in drive_raw:
        drivetrain = "RWD"
    elif "4WD" in drive_raw or "4X4" in drive_raw or drive_raw.strip() == "4":
        drivetrain = "4X4"
    ...
    row = {
        ...
        "stock_number": stock,      # dealer stock number kept as its own field
        "title": title,
